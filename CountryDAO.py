...
class CountryDAO:
    ...
    def findByRegion(self,limit):
        ...
        values = (limit,)
        cursor = self.getcursor()
        sql="select c.country,c.capital,r.region,r.literacy,r.phone from country c inner join regionInfo r on c.country = r.country limit %s"
        cursor.execute(sql)
        results = cursor.fetchall()
        returnArray = []
        print(results)
